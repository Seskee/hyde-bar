      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8 items-start mb-16 text-center md:text-left">
          ...
          <div className="flex flex-col items-center md:items-start gap-4">
            ...
            <div className="flex items-center gap-3 text-white/70 text-[11px] uppercase tracking-[0.2em] font-normal hover:text-white transition-colors cursor-pointer">
          ...
          <div className="flex flex-col items-center justify-center gap-8 order-first md:order-0">
            ...
            <a href={CONTACT.googleReviewsUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 group -mt-2.5 cursor-pointer">
              ...
              <span className="text-white/70 text-[10px] uppercase tracking-[0.3em] font-normal group-hover:text-white transition-colors duration-500">{dict.ratingText}</span>
            </a>
          </div>
          
          <div className="flex flex-col items-center md:items-end gap-4">
            <h4 className="text-gold text-[11px] uppercase tracking-[0.4em] font-medium mb-2">{dict.inquiry}</h4>
            <a href={`tel:${CONTACT.phone.replace(/\s/g, '')}`} className="text-white/70 text-[11px] uppercase tracking-[0.2em] font-normal hover:text-white transition-colors">{CONTACT.phone}</a>
            <a href={`mailto:${CONTACT.email}`} className="text-white/70 text-[11px] uppercase tracking-[0.2em] font-normal hover:text-white transition-colors">{CONTACT.email}</a>
          </div>
        </div>
        
        <div className="pt-10 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-[10px] text-gold uppercase tracking-[0.6em] opacity-80">{dict.established}</p>
          <div className="flex gap-8 text-[10px] text-white/50 uppercase tracking-[0.3em]">
            ...
            <span className="hidden md:inline font-normal text-white/30 italic tracking-widest">{dict.designed}</span>
          </div>
        </div>
