...
export default function LocationSection({ dict }: { dict: LocationDict }) {
  ...
  return (
    <section id="location" className="py-32 px-6 bg-hyde-bg overflow-hidden reveal relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-32 items-start">
          ...
          <div className="space-y-16">
            <div>
              ...
              <div onClick={() => setShowMapMenu(true)} className="flex items-start gap-4 text-white/40 group cursor-pointer hover:text-white transition-colors duration-500">
                <MapPin className="w-5 h-5 text-gold shrink-0 mt-1" />
                <p className="text-[11px] uppercase tracking-[0.25em] leading-relaxed font-light border-b border-transparent group-hover:border-gold/30 pb-1 transition-all">{CONTACT.address}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-16 border-t border-white/5 pt-16">
              <div className="space-y-8">
                <div className="flex items-center gap-3">
                  <Clock className="w-4 h-4 text-gold opacity-50" /><h4 className="text-gold text-[10px] uppercase tracking-[0.4em] font-medium">{dict.barTitle}</h4>
                </div>
              ...
              <div className="space-y-8">
                <div className="flex items-center gap-3">
                  <Utensils className="w-4 h-4 text-gold opacity-50" /><h4 className="text-gold text-[10px] uppercase tracking-[0.4em] font-medium">{dict.kitchenTitle}</h4>
                </div>
                <ul className="text-[10px] uppercase tracking-[0.25em] text-white/30 space-y-5 font-light">
          ...
          <div className="h-[500px] md:h-[750px] w-full relative border border-white/5 overflow-hidden group shadow-2xl bg-hyde-bg">
            <iframe src={CONTACT.googleMapsEmbed} title="HYDE bar & dine lokacija" width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="w-full h-full opacity-70 group-hover:opacity-50 transition-opacity duration-700" />
            <div className="absolute inset-0 flex items-center justify-center cursor-pointer" onClick={() => setShowMapMenu(true)}>
              <MagneticWrapper>
                <div className="bg-hyde-bg/80 border border-gold/30 backdrop-blur-md px-8 py-4 flex items-center gap-3 transform group-hover:scale-105 transition-all duration-500 hover:border-gold/60">
                  <Navigation className="w-4 h-4 text-gold" /><span className="text-gold text-[10px] uppercase tracking-widest">Navigacija</span>
                </div>
              </MagneticWrapper>
            </div>
          </div>
      ...
      {/* MODAL — POPRAVLJEN Z-INDEX (10020) I CENTRIRANJE (h-[100dvh]) */}
      <div className={`fixed inset-0 z-[10020] flex items-center justify-center p-6 transition-all duration-500 ${showMapMenu ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
        ...
        <div className={`relative w-full max-w-sm bg-hyde-bg border border-white/10 p-8 shadow-2xl transform transition-all duration-700 ${showMapMenu ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}>
          <button onClick={() => setShowMapMenu(false)} className="absolute top-4 right-4 text-white/40 hover:text-gold transition-colors"><X size={20} strokeWidth={1} /></button>
          <div className="text-center mb-8 mt-2">
            ...
            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-3">Odaberite željenu mapu</p>
          </div>
          <div className="space-y-4">
            <a href={CONTACT.googleMapsPin} target="_blank" rel="noopener noreferrer" className="block w-full text-center border border-white/10 hover:border-gold hover:bg-gold/5 py-4 transition-all duration-300"><span className="text-[11px] text-white uppercase tracking-[0.3em]">Google Maps</span></a>
            <a href={appleMapsLink} target="_blank" rel="noopener noreferrer" className="block w-full text-center border border-white/10 hover:border-gold hover:bg-gold/5 py-4 transition-all duration-300"><span className="text-[11px] text-white uppercase tracking-[0.3em]">Apple Maps</span></a>
          </div>
        </div>
      </div>
    </section>
